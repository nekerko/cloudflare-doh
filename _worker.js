...
const DEFAULT_PATH_MAPPINGS = {
	'/google': {
		...
		pathMapping: {
			'/query-dns': '/dns-query',   // for wireformat requests
			'/dns-query': '/resolve'      // for JSON requests (your use case)
		},
	},
	'/cloudflare': {
		...
		pathMapping: {
			'/query-dns': '/dns-query',
			'/dns-query': '/dns-query',   // Cloudflare's /dns-query supports both wireformat and JSON
		},
	},
  ...
  <body>
	  ...
	  <div class="container">
		  ...
		  <section>
			  ...
			  <div class="example">
				  <code>https://doh-proxy.example.com/google/dns-query?name=example.com</code>
			  </div>

			  <h3>转发到 Cloudflare DoH 服务</h3>
			  <div class="example">
				  <code>https://doh-proxy.example.com/cloudflare/dns-query?name=example.com</code>
			  </div>

			  <h3>HTTP 请求示例</h3>
			  <pre><code>curl -H "accept: application/dns-json" "https://doh-proxy.example.com/google/dns-query?name=example.com&type=A"</code></pre>
		  </section>
	  ...
	  "pathMapping": {
		"/dns-query": "/dns-query"
			  ...
			  <div class="card">
				  ...
				  <ol>
					  ...
					  <li>选择"自定义"选项，并输入以下 URL（以 Google 为例）：<br>
						  <code>https://your-worker-domain.com/google/dns-query</code>
					  </li>
					  <li>点击"确定"保存设置</li>
				  </ol>
			  </div>
			  ...
			  <div class="card">
				  ...
				  <ol>
					  ...
					  <li>选择"自定义"选项，并输入以下 URL（以 Cloudflare 为例）：<br>
						  <code>https://your-worker-domain.com/cloudflare/dns-query</code>
					  </li>
				  </ol>
		  ...
		  <section class="cta">
			  ...
			  <a href="https://github.com/jqknono/cloudflare-doh" class="btn" target="_blank">获取代码</a>
		  </section>
	  </div>

	  <footer>
		  <div class="container">
			  <p>&copy; 2025 Cloudflare DoH 转发代理 | 开源项目 | 基于 Cloudflare Workers</p>
		  </div>
	  </footer>
  </body>
...
function getPathMappings(env) {
	if (env && env.DOMAIN_MAPPINGS) {
		try {
			...
			console.error('Failed to parse DOMAIN_MAPPINGS, using defaults', e);
		}
	}
